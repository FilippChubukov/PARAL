    ...
    class Program
    {
        static void Main()
        {
            ...
            if(!tree.IsBst(tree.Root))
                throw new Exception("Структура дерева нарушена после последовательной вставки!");
            ...
            if(!tree.IsBst(tree.Root))
                throw new Exception("Структура дерева нарушена после последовательного удаления!");
            ...
            if(!pTree.IsBst(pTree.Root))
                throw new Exception("Структура дерева нарушена после параллельной вставки!");
            ...
            if(!pTree.IsBst(pTree.Root))
                throw new Exception("Структура дерева нарушена после параллельного удаления!");
